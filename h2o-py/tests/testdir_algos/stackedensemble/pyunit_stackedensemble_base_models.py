...
def test_stacked_ensemble_accepts_mixed_definition_of_base_models():
    ...
    * list of Gridsearches and models
    ...
    assert sorted(se1.base_models) == sorted(se2.base_models)
    ...
    assert sorted(se3.base_models) == sorted(se4.base_models)
    ...
    assert sorted(se5.base_models) == sorted(se6.base_models)
    ...
    assert sorted(se6.base_models) == sorted(se7.base_models)

    # Check that we validate base models
    se8 = H2OStackedEnsembleEstimator(base_models=se6.base_models + [data.train.frame_id])
    try:
        se8.train(data.x, data.y, data.train)
    except H2OResponseError as e:
        assert "Don't know yet how to use class water.fvec.Frame as a base model" in str(e)
    else:
        assert False, "Should have gotten an exception here"
